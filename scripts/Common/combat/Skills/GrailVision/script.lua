-- 
CREATURE_BTD_GRAIL_VISION_CASTER = 212
...
function GrailVision_Start(side)
  ...
  local curr_count = len(creatures) > 2 and grail_vision_common[side].stacks_count or len(creatures)
  ...
  --
    ...
    if pcall(AddCreature, side, CREATURE_BTD_GRAIL_VISION_CASTER, 6, -1, -1, nil, helper) then
        while not exist(helper) do
            sleep()
        end
        startThread(playAnimation, GetHero(side), "buff", ONESHOT)
        for i, stack in stacks_to_cast do
            pcall(UnitCastAimedSpell, helper, 175, stack)
        end
        sleep(10)
        pcall(removeUnit, helper)
        repeat
