...
function DefendUsAll_DefileMagic(side, key, creature)
    local uniq_key = 'defile_magic_'..side..'_'..key;
    
    if pcall(AddCreature, side, CREATURE_GOBLIN_DEFILER, 10000, -1, -1, nil, uniq_key) then
        while not exist(uniq_key) do
            sleep()
        end
        ...
        pcall(commandDoSpecial, uniq_key, SPELL_ABILITY_DEFILE_MAGIC, x, y)
    end;
...
function DefendUsAll_Start(side)
    for i, creature in GetCreatures(side) do
        ...
        if type == CREATURE_GOBLIN_TRAPPER and (not defend_us_all[side].trapper_cast_used) then
            ...
            for _, enemy_creature in GetCreatures(1 - side) do
            ...
            if n > 0 then
                for _, target in possible_stacks_to_cast do
                    ...
                    DefendUsAll_SetSnare(side, 'defend_us_all_trap_under_creature_', x, y);
            ...
            for index = 1, RANDOM_COUNT_MIDDLE_TRAP do
                DefendUsAll_SetSnare(side, 'defend_us_all_middle_trap_'..index);
            end;
        ...
        if type == CREATURE_GOBLIN_DEFILER and (not defend_us_all[side].defiler_cast_used) then
            ...
            if n > 0 then
                for _, stack in possible_stacks_to_cast do
                    DefendUsAll_DefileMagic(side, helper, stack)
                end
            end
            --
